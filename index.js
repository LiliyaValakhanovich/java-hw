...
const getTransformedNumbers = (numArray, degree) => {
  const newArray=numArray.map((item)=>Math.pow(item, degree));
    return newArray;
 };
...
const getTransformedText = (text) => {
 return text
 .split('. ')
 .map(function array(item) {return `${item[0].toUpperCase()}${item.slice(1)}`})
 .join('. ');
};
...
const getPositiveIntegers = (array) => {
  return array.filter(function number(item) {if (Number.isFinite(item)){if(Math.sign(item)===1 || Math.sign(item)===0){return item}}});
};
...
const isNegativeNumbersInArray = (array) => {
  return array.some(item=>Math.sign(item)===-1);
};
